import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, bigint, varchar, datetime, unique, decimal, mysqlEnum, int, index, foreignKey, tinyint, serial, text, timestamp, date, check } from "drizzle-orm/mysql-core"
import { sql } from "drizzle-orm"

export const accessibilities = mysqlTable("accessibilities", {
...
(table) => [
	primaryKey({ columns: [table.id], name: "accessibilities_id"}),
]);

export const adeptAccess = mysqlTable("adept_access", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "adept_access_id"}),
]);

export const airtimeProviders = mysqlTable("airtime_providers", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
export const airtimeTransactions = mysqlTable("airtime_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	...
	balanceBefore: decimal("balance_before", { precision: 15, scale: 2 }).default('0.00').notNull(),
	balanceAfter: decimal("balance_after", { precision: 15, scale: 2 }).default('0.00').notNull(),
},
(table) => [
...
export const appSettings = mysqlTable("app_settings", {
	...
	verificationPlatform: mysqlEnum("verification_platform", ['dojah','mono']).default('dojah').notNull(),
	paymentPlatform: mysqlEnum("payment_platform", ['flutterwave','paystack','irecharge']).notNull(),
	minimumAmount: decimal("minimum_amount", { precision: 8, scale: 0 }),
	irchgTokenTtl: datetime("irchg_token_ttl", { mode: 'string'}),
	irchgChargeRate: decimal("irchg_charge_rate", { precision: 5, scale: 0 }).default('50').notNull(),
	irchgTokenLogin: varchar("irchg_token_login", { length: 60 }),
	minimumAmountToPhone: decimal("minimum_amount_to_phone", { precision: 8, scale: 2 }).default('200.00').notNull(),
	withdrawalChargeRate: decimal("withdrawal_charge_rate", { precision: 8, scale: 0 }).default('20').notNull(),
	createdAt: datetime({ mode: 'string'}).default(sql`(now())`).notNull(),
	noKycMaxAmount: decimal("no_kyc_max_amount", { precision: 8, scale: 2 }).default('20000.00').notNull(),
	basicTransferAmount: decimal("basic_transfer_amount", { precision: 10, scale: 2 }).default('20000.00').notNull(),
	intermediateTransferAmount: decimal("intermediate_transfer_amount").default('5000000').notNull(),
	minAmountToPhone: decimal("min_amount_to_phone", { precision: 10, scale: 2 }).default('200.00').notNull(),
	minAmountToWallet: decimal("min_amount_to_wallet", { precision: 10, scale: 2 }).default('100.00').notNull(),
	nonBlueChargeRate: decimal("non_blue_charge_rate", { precision: 10, scale: 2 }).default('50.00').notNull(),
	blueChargeRate: decimal("blue_charge_rate", { precision: 10, scale: 2 }).default('0.00').notNull(),
	...
	powerPlatform: mysqlEnum("power_platform", ['irecharge','vtpass','buypower']).notNull(),
	airtimePlatform: mysqlEnum("airtime_platform", ['irecharge','vtpass','buypower']).notNull(),
	dataPlatform: mysqlEnum("data_platform", ['irecharge','vtpass','buypower']).notNull(),
	tvPlatform: mysqlEnum("tv_platform", ['irecharge','vtpass','buypower']).notNull(),
},
(table) => [
...
(table) => [
	primaryKey({ columns: [table.id], name: "banks_id"}),
]);
...
export const billMatesFavourites = mysqlTable("bill_mates_favourites", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
	providerId: bigint("provider_id", { mode: "number", unsigned: true }),
	providerType: mysqlEnum("provider_type", ['power_providers','data_providers','tv_providers','airtime_providers']).notNull(),
...
export const billMatesTransactions = mysqlTable("bill_mates_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	amount: decimal({ precision: 15, scale: 2, unsigned: true }).notNull(),
	status: mysqlEnum(['successful','pending','failed','in-progress','processing']).notNull(),
...
export const blueAdminRoles = mysqlTable("blue_admin_roles", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	name: varchar({ length: 50 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(now())`),
},
(table) => [
	primaryKey({ columns: [table.id], name: "blue_admin_roles_id"}),
]);

export const blueAdmins = mysqlTable("blue_admins", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "blue_admins_id"}),
]);

export const businessBranches = mysqlTable("business_branches", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }).references(() => businessProfiles.id, { onDelete: "cascade" } ),
...
export const businessDataTransactions = mysqlTable("business_data_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }).references(() => businessProfiles.id, { onDelete: "set null" } ),
	branchId: bigint("branch_id", { mode: "number", unsigned: true }).references(() => businessBranches.id, { onDelete: "set null" } ),
	staffId: bigint("staff_id", { mode: "number", unsigned: true }).references(() => staffProfiles.id, { onDelete: "set null" } ),
...
export const businessDevices = mysqlTable("business_devices", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "business_devices_id"}),
]);

export const businessFees = mysqlTable("business_fees", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "business_fees_id"}),
]);
...
export const businessOnboardingForms = mysqlTable("business_onboarding_forms", {
	...
	monthlyTransactions: varchar("monthly_transactions", { length: 100 }).notNull(),
...
export const businessPaymentTransactions = mysqlTable("business_payment_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
	...
	clientUsed: mysqlEnum("client_used", ['desktop','mobile']).default('mobile'),
	dueDate: datetime("due_date", { mode: 'string'}),
	paymentMode: mysqlEnum("payment_mode", ['blue-user','phone','offline','withdrawal','wallet_topup','card','bank_transfer']).notNull(),
	status: mysqlEnum(['in-progress','pending','successful','failed','reversed']).default('pending').notNull(),
	transactionId: varchar("transaction_id", { length: 30 }).notNull(),
	...
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
	receiverType: mysqlEnum("receiver_type", ['personal','business','business-personal']),
...
(table) => [
	...
	primaryKey({ columns: [table.id], name: "business_payment_transactions_id"}),
	unique("unique_bus_transaction").on(table.transactionId, table.businessId),
]);

export const businessPins = mysqlTable("business_pins", {
...
export const businessPowerTransactions = mysqlTable("business_power_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
	branchId: bigint("branch_id", { mode: "number", unsigned: true }),
	staffId: bigint("staff_id", { mode: "number", unsigned: true }),
	...
	serviceCharge: decimal("service_charge", { precision: 10, scale: 2 }).default('100.00'),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
...
export const businessRecoveryInfos = mysqlTable("business_recovery_infos", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
...
export const businessSecurityQuestions = mysqlTable("business_security_questions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
	createdAt: datetime("created_at", { mode: 'string'}),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
...
export const businessSettlementAccounts = mysqlTable("business_settlement_accounts", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "business_settlement_accounts_id"}),
	unique("unique_business_id").on(table.businessId),
]);

export const businessShareholders = mysqlTable("business_shareholders", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }).notNull(),
	name: varchar({ length: 255 }).notNull(),
	address: varchar({ length: 255 }),
...
export const businessTransactionHistories = mysqlTable("business_transaction_histories", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
	branchId: bigint("branch_id", { mode: "number", unsigned: true }),
	staffId: bigint("staff_id", { mode: "number", unsigned: true }),
	...
	status: mysqlEnum(['successful','pending','failed','in-progress']).notNull(),
	paymentMode: mysqlEnum("payment_mode", ['blue-user','phone','offline','withdrawal','card','bank_transfer','airtime','data','power','cable-tv']).notNull(),
	type: mysqlEnum(['credit','debit']).notNull(),
	clientUsed: mysqlEnum("client_used", ['desktop','mobile']).default('mobile').notNull(),
	serviceCharge: decimal("service_charge", { precision: 10, scale: 2 }).default('0.00').notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`).notNull(),
	balanceBefore: decimal("balance_before", { precision: 15, scale: 2 }).default('0.00').notNull(),
	balanceAfter: decimal("balance_after", { precision: 15, scale: 2 }).default('0.00').notNull(),
},
(table) => [
	...
	primaryKey({ columns: [table.id], name: "business_transaction_histories_id"}),
]);

export const businessTvTransactions = mysqlTable("business_tv_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
	branchId: bigint("branch_id", { mode: "number", unsigned: true }),
	staffId: bigint("staff_id", { mode: "number", unsigned: true }),
...
export const businessUsers = mysqlTable("business_users", {
	...
	verified: tinyint({ unsigned: true }).default(0),
	level: int().default(1),
	notificationStatus: tinyint("notification_status", { unsigned: true }).default(0),
	resetCredentialStatus: tinyint("reset_credential_status", { unsigned: true }).default(0),
	password: varchar({ length: 255 }),
	passwordChangedAt: varchar("password_changed_at", { length: 255 }),
	kyc: mysqlEnum(['basic','intermediate','pro']).default('basic').notNull(),
	businessProfileCompleted: tinyint("business_profile_completed", { unsigned: true }).default(0),
	businessDetailsCompleted: tinyint("business_details_completed", { unsigned: true }).default(0),
	businessKycCompleted: tinyint("business_kyc_completed", { unsigned: true }).default(0),
	proofOfAddressVerified: tinyint("proof_of_address_verified", { unsigned: true }).default(0),
	autoWithdrawalEnabled: tinyint("auto_withdrawal_enabled", { unsigned: true }).default(0),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
},
(table) => [
...
export const businessWalletSnapshots = mysqlTable("business_wallet_snapshots", {
	businessId: bigint("business_id", { mode: "number", unsigned: true }),
	type: mysqlEnum(['credit','debit']).notNull(),
	createdAt: datetime("created_at", { mode: 'string', fsp: 6 }).default(sql`(CURRENT_TIMESTAMP(6))`).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string', fsp: 6 }).default(sql`(CURRENT_TIMESTAMP(6))`).notNull(),
...
export const dashmeIdempotencyRecords = mysqlTable("dashme_idempotency_records", {
	id: int().autoincrement().notNull(),
...
(table) => [
	...
	index("transaction_id").on(table.transactionId, table.userId, table.operationType),
	primaryKey({ columns: [table.id], name: "dashme_idempotency_records_id"}),
]);

export const dashmeTransactions = mysqlTable("dashme_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
	amount: decimal({ precision: 15, scale: 2 }).notNull(),
	orderReference: varchar("order_reference", { length: 50 }),
	...
	dueDate: datetime("due_date", { mode: 'string'}),
	status: mysqlEnum(['pending','sent','successful','failed','reversed']).default('pending').notNull(),
	serviceCharge: decimal("service_charge", { precision: 10, scale: 2 }).default('0.00').notNull(),
	narration: varchar({ length: 255 }),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
...
export const dashmeTransactionsAttemptLogs = mysqlTable("dashme_transactions_attempt_logs", {
	id: int().autoincrement().notNull(),
	userId: int("user_id").notNull(),
	transactionId: varchar("transaction_id", { length: 255 }).notNull(),
...
(table) => [
	...
	primaryKey({ columns: [table.id], name: "dashme_transactions_attempt_logs_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "data_packages_id"}),
]);

export const dataProviders = mysqlTable("data_providers", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	name: varchar({ length: 20 }).notNull(),
	slug: varchar({ length: 20 }).notNull(),
	image: varchar({ length: 200 }).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "data_providers_id"}),
]);

export const dataTransactions = mysqlTable("data_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	receiver: varchar({ length: 25 }).notNull(),
	provider: varchar({ length: 80 }).notNull(),
...
export const deletedAccounts = mysqlTable("deleted_accounts", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
export const flaggedUsers = mysqlTable("flagged_users", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
export const irchgBanks = mysqlTable("irchg_banks", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	code: varchar({ length: 20 }).notNull(),
	bankCode: varchar("bank_code", { length: 20 }),
	name: varchar({ length: 150 }).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "irchg_banks_id"}),
]);

export const irechargeDataPackages = mysqlTable("irecharge_data_packages", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	code: varchar({ length: 20 }).notNull(),
	title: varchar({ length: 100 }).notNull(),
	validity: varchar({ length: 100 }),
	price: decimal({ precision: 12, scale: 2, unsigned: true }).default('0.00').notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
},
(table) => [
...
export const irechargePowerProviders = mysqlTable("irecharge_power_providers", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	description: varchar({ length: 100 }).notNull(),
	slug: varchar({ length: 15 }).notNull(),
	code: varchar({ length: 100 }).notNull(),
	minimumValue: decimal("minimum_value", { precision: 12, scale: 2, unsigned: true }).default('0.00'),
	maximumValue: decimal("maximum_value", { precision: 12, scale: 2, unsigned: true }).default('0.00'),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
},
(table) => [
...
export const irechargeTvPackages = mysqlTable("irecharge_tv_packages", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	title: varchar({ length: 100 }).notNull(),
...
export const paymentLinks = mysqlTable("payment_links", {
	...
	status: mysqlEnum(['failed','sent','processing','fulfilled','cancelled','withdrawn','reversed']).default('sent'),
...
(table) => [
	primaryKey({ columns: [table.id], name: "payment_links_id"}),
]);
...
export const paystackBanks = mysqlTable("paystack_banks", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	code: varchar({ length: 20 }).notNull(),
	bankCode: varchar("bank_code", { length: 20 }),
	name: varchar({ length: 150 }).notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "paystack_banks_id"}),
]);

export const pilotTesters = mysqlTable("pilot_testers", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
export const powerProviders = mysqlTable("power_providers", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "power_providers_id"}),
]);

export const powerTransactions = mysqlTable("power_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	meterType: mysqlEnum("meter_type", ['prepaid','postpaid']).notNull(),
	receiver: varchar({ length: 25 }).notNull(),
	...
	meterCategory: mysqlEnum("meter_category", ['MD','NON-MD']),
	paymentMode: mysqlEnum("payment_mode", ['wallet','card']),
	status: mysqlEnum(['pending','successful','failed','processing']).default('pending').notNull(),
	accessToken: varchar("access_token", { length: 40 }),
	vendRequestBody: varchar("vend_request_body", { length: 1000 }),
	transactionId: varchar("transaction_id", { length: 30 }).notNull(),
	phone: varchar({ length: 15 }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "reasons_id"}),
]);

export const recoveryInfos = mysqlTable("recovery_infos", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
...
export const reportPreferences = mysqlTable("report_preferences", {
	...
	email: varchar({ length: 255 }).notNull(),
	isActive: tinyint("is_active").default(1).notNull(),
	scheduleTime: varchar("schedule_time", { length: 10 }).default('23:00').notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).default(sql`(now())`).notNull(),
...
export const securityQuestions = mysqlTable("security_questions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
...
(table) => [
	primaryKey({ columns: [table.id], name: "sms_logs_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "staff_roles_id"}),
]);

export const tempAccounts = mysqlTable("temp_accounts", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "temp_accounts_id"}),
]);

export const tempRecoveryPhones = mysqlTable("temp_recovery_phones", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
export const transactionHistories = mysqlTable("transaction_histories", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	transactionId: bigint("transaction_id", { mode: "number", unsigned: true }),
	receiverName: varchar("receiver_name", { length: 250 }),
	amount: decimal({ precision: 15, scale: 2, unsigned: true }).notNull(),
	status: mysqlEnum(['successful','pending','failed','in-progress','refunded']).notNull(),
	paymentMode: mysqlEnum("payment_mode", ['blue-user','phone','wallet_topup','offline','withdrawal','card','bank-transfer','airtime','data','power','cable-tv']).notNull(),
	type: mysqlEnum(['credit','debit']).notNull(),
	senderName: varchar("sender_name", { length: 100 }),
	clientUsed: mysqlEnum("client_used", ['desktop','mobile']).default('mobile').notNull(),
	transactionReference: varchar("transaction_reference", { length: 20 }),
...
export const tvPackages = mysqlTable("tv_packages", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	provider: varchar({ length: 10 }).notNull(),
	code: varchar({ length: 20 }).notNull(),
	name: varchar({ length: 100 }).notNull(),
	amount: decimal({ precision: 12, scale: 2, unsigned: true }).default('0.00').notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`).notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "tv_packages_id"}),
]);

export const tvProviders = mysqlTable("tv_providers", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	name: varchar({ length: 80 }).notNull(),
	slug: varchar({ length: 10 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
...
(table) => [
	primaryKey({ columns: [table.id], name: "tv_providers_id"}),
]);

export const tvTransactions = mysqlTable("tv_transactions", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	receiver: varchar({ length: 25 }).notNull(),
	provider: varchar({ length: 80 }).notNull(),
...
export const userCardDetails = mysqlTable("user_card_details", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }).notNull(),
	email: varchar({ length: 255 }).notNull(),
...
export const users = mysqlTable("users", {
	...
	type: mysqlEnum(['personal','business','business_personal']).default('personal'),
...
export const ussdWithdrawals = mysqlTable("ussd_withdrawals", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	bankCode: varchar("bank_code", { length: 20 }),
	...
	status: mysqlEnum(['in-progress','pending','successful','failed']).default('pending').notNull(),
	verificationRequestBody: varchar("verification_request_body", { length: 500 }),
	verificationResponseBody: varchar("verification_response_body", { length: 1000 }),
...
(table) => [
	primaryKey({ columns: [table.id], name: "ussd_withdrawals_id"}),
]);
...
(table) => [
	...
	unique("unique_user_transaction_type").on(table.userId, table.transactionReference, table.type),
...
export const wallets = mysqlTable("wallets", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
	userId: bigint("user_id", { mode: "number", unsigned: true }),
	balance: decimal({ precision: 15, scale: 2, unsigned: true }).default('0.00'),
	walletId: varchar("wallet_id", { length: 20 }).notNull(),
	createdAt: datetime("created_at", { mode: 'string'}).notNull(),
	updatedAt: datetime("updated_at", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
	walletCode: varchar("wallet_code", { length: 20 }).notNull(),
...
export const withdrawCredentials = mysqlTable("withdraw_credentials", {
	id: bigint({ mode: "number", unsigned: true }).autoincrement().notNull(),
...
export const withdrawIdempotencyRecords = mysqlTable("withdraw_idempotency_records", {
	id: int().autoincrement().notNull(),
	transactionId: varchar("transaction_id", { length: 255 }).notNull(),
	userId: int("user_id").notNull(),
	operationType: varchar("operation_type", { length: 50 }).notNull(),
...
(table) => [
	index("created_at").on(table.createdAt),
	index("transaction_id").on(table.transactionId, table.userId, table.operationType),
	primaryKey({ columns: [table.id], name: "withdraw_idempotency_records_id"}),
]);
